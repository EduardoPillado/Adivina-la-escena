...
<body class="fondo-opaco-amarillo">
    ...
    <div class="flex flex-col sm:flex-row justify-center space-x-0 sm:space-x-10 mb-8">
        <form action="{{route('multimedia.insertar')}}" method="post" enctype="multipart/form-data">
            @csrf

            <label>Escena:</label>

            <input type="file" id="multimediaFile" name="nombreMultimedia" accept="video/*" style="display: none;" onchange="loadFile(event)" required>
            ...
            <option value="">Selecciona la categoría</option>
                ...
                @foreach ($datos_categoria as $dato)
                    <option value="{{$dato->pkCategoria}}">{{$dato->nombreCategoria}}</option>
                @endforeach
            </select>

            <label for="">Opción 1 (opción correcta)</label>
            <input type="text" name="nombreOpcion1" required>

            <label for="">Opción 2</label>
            <input type="text" name="nombreOpcion2" required>

            <label for="">Opción 3</label>
            <input type="text" name="nombreOpcion3" required>

            <label for="">Opción 4</label>
            <input type="text" name="nombreOpcion4" required>
